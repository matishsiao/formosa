<?php
/**
 * Copyright (c) 2016, Matis Hsiao
 * All rights reserved.
 * @author: Matis Hsiao
 *
 * Formosa PHP client SDK.
 */

class FormosaException extends Exception
{
}

class FormosaTimeoutException extends FormosaException
{
}
 ...
 */
class SimpleFormosa extends Formosa
{
...
class Formosa
{
	...
	function __construct($host, $port, $timeout_ms=2000){
		...
		if(!$this->sock){
			throw new FormosaException("$errno: $errstr");
		}
	 ...
	 * will not return a Formosa_Response object.
	...
	function __call($cmd, $params=array()){
		...
		try{
			if($this->send_req($cmd, $params) === false){
				$resp = new Formosa_Response('error', 'send error');
			}else{
				$resp = $this->recv_resp($cmd, $params);
			}
		}catch(FormosaException $e){
			if($this->_easy){
				throw $e;
			}else{
				$resp = new Formosa_Response('error', $e->getMessage());
			}
		}
	...
	private function recv_resp($cmd, $params){
		$resp = $this->recv();
		if($resp === false){
			return new Formosa_Response('error', 'Unknown error');
		}else if(!$resp){
			return new Formosa_Response('disconnected', 'Connection closed');
		}
		if($resp[0] == 'noauth'){
			$errmsg = isset($resp[1])? $resp[1] : '';
			return new Formosa_Response($resp[0], $errmsg);
		}
		switch($cmd){
			case 'dbsize':
			case 'ping':
			...
			case 'zremrangebyscore':
				if($resp[0] == 'ok'){
					...
					return new Formosa_Response($resp[0], $val);
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new Formosa_Response($resp[0], $errmsg);
				}
			...
			case 'qback':
				if($resp[0] == 'ok'){
					if(count($resp) == 2){
						return new Formosa_Response('ok', $resp[1]);
					}else{
						return new Formosa_Response('server_error', 'Invalid response');
					}
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new Formosa_Response($resp[0], $errmsg);
			...
			case 'qpop_back':
				if($resp[0] == 'ok'){
					...
					if($size <= 1){
						...
						return new Formosa_Response('ok', $data);
					}
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new Formosa_Response($resp[0], $errmsg);
			...
			case 'qslice':
				if($resp[0] == 'ok'){
					...
					return new Formosa_Response($resp[0], $data);
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new Formosa_Response($resp[0], $errmsg);
				}
			...
			case 'zexists':
				if($resp[0] == 'ok'){
					if(count($resp) == 2){
						return new Formosa_Response('ok', (bool)$resp[1]);
					}else{
						return new Formosa_Response('server_error', 'Invalid response');
					}
				}else{
			...
			default:
				return new Formosa_Response($resp[0], array_slice($resp, 1));
		}
		return new Formosa_Response('error', 'Unknown command: $cmd');
	}
	...
	private function send($data){
		...
		try{
			while(true){
				...
				if($ret === false){
					$this->close();
					throw new FormosaException('Connection lost');
				}
				$s = substr($s, $ret);
				if(strlen($s) == 0){
					break;
			...
			$this->close();
			throw new FormosaException($e->getMessage());
		}
		return $ret;
	}
			...
			if($ret === null){
				...
				if($data === false || $data === ''){
					if(feof($this->sock)){
						...
						throw new FormosaTimeoutException('Connection timeout');
					}
				}
				$this->recv_buf .= $data;
				...
				if(strlen($line) == 0){ // head end
					...
					if ($ret[0] == "zip") {
						...
						while (true) {
							...
							$rpos += 1;
							$line = substr($result, 0, $rpos);
							$idx = intval($line);
							if ($idx > 0) {
								$str = substr($result,$rpos,$idx);
								array_push($resp,$str);
								$result = substr($result, $rpos + $idx);
							} else {
								$result = substr($result, $rpos );
							}
						}
						$useTime = microtime(true) - $start;
						echo "zip use:$useTime\n";
						$ret = $resp;
					}
					$this->resp = array();
					return $ret;
